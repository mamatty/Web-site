...
?>
    ...
    <div class="main">
        ...
        if(isset($_GET['search'])){
            $params = $_GET['search'];
            $query1=$mysqli->prepare("SELECT id_user, name, surname, email FROM user WHERE surname = ? ORDER BY id_user DESC LIMIT $from_record_num,$records_per_page");
            $query1->bind_param('s',$params);
            $query1->execute();
            $query1->store_result();
            ...
            if ($result > 0){
                ...
                $query1->bind_result($id_user, $name, $surname, $email);
                ...
                $statement = mysqli_prepare($mysqli,"SELECT * FROM user WHERE surname = '".$_GET['search']."'");
