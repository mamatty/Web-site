...
<div class="main">
    ...
    <?php
    $query_user = $mysqli->prepare("SELECT name,surname FROM user WHERE id_user = ?");
    $query_user->bind_param("i",$id);
    $query_user->execute();
    $result = $query_user->get_result();
    if ($query_user->execute() === true) {
    ...
    <br>
        ...
        $query = $mysqli->prepare("SELECT id_schedule, name, details, start_date, end_date, num_days, objective FROM schedules WHERE id_user = ? ORDER BY end_date DESC LIMIT $from_record_num,$records_per_page");
        ...
        if ($result > 0) {
            ...
            $query->bind_result($id_schedule, $name, $details, $start_date, $end_date, $num_days, $objective);
            ...
            $statement = mysqli_prepare($mysqli, 'SELECT * FROM schedules WHERE id_user = ? ');
