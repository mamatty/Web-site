 ...
 */
    ...
    if($_POST){
        ...
        if(isset($_POST["argument"])){
            ...
            $argument = ['All','Multicast'];
        }

        #TODO capire se esiste un modo di inviare immagini che non siano un URL.

        $now = new DateTime();
        $attuale = $now;
        $datatime = $attuale->format('Y/m/d');

        if($_POST['destination'] == 'token'){
            $query = $mysqli->prepare("INSERT INTO messages (title, body, send_date, destination) VALUES (?,?,?,?)");
            $query->bind_param('ssss',$title,$body,$datatime, $argument[0]);
        ...
        elseif($_POST['destination'] == 'multicast'){
            $query = $mysqli->prepare("INSERT INTO messages (title, body, send_date, destination) VALUES (?,?,?,?)");
            $query->bind_param('ssss',$title,$body,$datatime, $argument[1]);
            if($query->execute()=== true){
                echo "<div class='alert alert-success'>Message has been saved.</div>";
            ...
            $query = $mysqli->prepare("INSERT INTO messages (title, body, send_date, destination) VALUES (?,?,?,?)");
            $query->bind_param('ssss',$title,$body,$datatime, $argument);
            if($query->execute()=== true){
                echo "<div class='alert alert-success'>Message has been saved.</div>";
                $load = true;
            }else{
        ...
        if ($load == true){
            $result = $send -> sendNotification($title, $body, null, $destination, $argument);
            ...
            <tr>
                ...
                <td>
                    <select id="destination" name="destination" onchange="destinationChange(this);">
                        <option value="token">Send to everybody</option>
